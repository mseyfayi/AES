...
public class AESEncoder {
    ...
    AESEncoder(String plain, String key) {
        this.plain = plain;
        this.key = key;
    }

    private int[][] subBytes(int[][] grid) {
        int[][] matrix = new int[4][4];
        for (int i = 0; i < 4; i++)
            for (int j = 0; j < 4; j++)
                matrix[i][j] = lookup(grid[i][j]);
        return matrix;
    }
    ...
    private int[][] shiftRows(int[][] grid) {
        int[][] matrix = new int[4][4];
        for (int i = 0; i < 4; i++)
            matrix[i] = leftShit(grid[i], i);
        return matrix;
    }

    private int[] leftShit(int[] arr, int sham) {
        int[] result = new int[4];
        for (int i = 0; i < 4; i++)
            result[i] = arr[(sham + i) % 4];
        return result;
    }
    ...
    String encrypt() {
        int[][] grid = createGridFromString(this.plain);
...
*/
        printGrid(grid, 0, "grid");

        int[][] roundKey = extractRoundKey(expandedKey, 0);
        ...
        grid = subBytes(grid);
        printGrid(grid, 10, "subBytes");

        grid = shiftRows(grid);
        printGrid(grid, 10, "shiftRows");

        roundKey = extractRoundKey(expandedKey, 10);
        printGrid(grid, 10, "roundKey");
    ...
    public static void main(String[] args) {
        ...
        AESEncoder encoder = new AESEncoder(plain, key);
        String cypher = encoder.encrypt();
